...
void SierpinskiPyramid::DrawTri(glm::mat4 Change)
{
	...
	glUniform4f(objColorLocation, Color.r, Color.g, Color.b, 1.0);
					...
					for (int l = 0; l < 3; ++l) {
						for (int m = 0; m < 3; ++m) {
							for (int n = 0; n < 3; ++n) {
								for (int o = 0; o < 3; ++o) {
									for (int p = 0; p < 3; ++p) {
										glm::mat4 temp = Change * tra[i] * sca * tra[j] * sca * tra[k] * sca * tra[l] * sca * tra[m] * sca * tra[n] * sca * tra[o] * sca * tra[p] * sca;
										DrawTri(temp);
									}
								}
							}
